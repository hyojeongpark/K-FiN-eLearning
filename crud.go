...
func CreateUser(db *sql.DB, req *http.Request) *CustomError {
	...
	motive := req.PostFormValue("motive")

	t := time.Now().Format("2006-01-02 15:04:05")
	// Create 2
	stmt, err := db.Prepare("insert into user (id, password, name, created,email,gender,motive) values (?, ?, ?, ?, ? ,? ,?)")
	checkError(err)
	defer stmt.Close()

	bs, err := bcrypt.GenerateFromPassword([]byte(password), bcrypt.MinCost)
	_, err = stmt.Exec(id, bs, name, t, email, gender, motive)
	if err != nil {
...
func ReadUserById(db *sql.DB, userId string) (User, error) {
	...
	var user = User{}
	for row.Next() {
		err := row.Scan(&user.Id, &user.Password, &user.Created, &user.Name, &user.Email, &user.Gender, &user.Motive)
		if err != nil {
			log.Fatal(err)
		}
	}
...
func ReadUser(db *sql.DB, req *http.Request) (User, *CustomError) {
	...
	if !rows.Next() {
		...
		_ = rows.Scan(&user.Id, &user.Password, &user.Created, &user.Name, &user.Email, &user.Gender, &user.Motive)
	}
